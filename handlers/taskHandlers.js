  ...
  async deleteTaskById(msg, taskId, bot) {
    try {
      ...
      if (task) {
        bot.sendMessage(
          msg.chat.id,
          `✅ Đã xoá task [${task.platform}]: *${task.content}* (ID: ${taskId})`,
          { parse_mode: "Markdown" }
        );
      } else {
  ...
  async listAllTasks(msg, bot) {
    try {
      ...
      if (tasks.length === 0) {
        ...
        const taskList = tasks
          .map((t) => `${t.id} - ${t.platform}: ${t.content}`)
          .join("\n");
        bot.sendMessage(
          msg.chat.id,
          `📌 *Danh sách Task còn lại:*\n${taskList}`,
  ...
  async filterTasksByPlatform(msg, platform, bot) {
    try {
      ...
      if (filteredTasks.length === 0) {
        bot.sendMessage(
          msg.chat.id,
          `📭 Không có task nào thuộc *${platform}*.`
        );
      } else {
  ...
  async sendDailyBugReport(bot, CHAT_ID) {
    try {
      ...
      platforms.forEach((platform) => {
        const platformBugs = tasks.filter(
          (task) => task.platform.toLowerCase() === platform.toLowerCase()
        );
